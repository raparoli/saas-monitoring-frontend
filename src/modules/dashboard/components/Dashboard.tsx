...
import { GlobalStat, IntegratedProduct, AIInsight } from '../types';
...
export function Dashboard({ onAcronisDetail }: DashboardProps) {
  ...
  const globalStats: GlobalStat[] = safeDashboardData ? [
    {
  ...
  const storageData = (safeDashboardData && Array.isArray(safeDashboardData.storageDistribution)) ? safeDashboardData.storageDistribution : [];
  const licenseUtilizationData = (safeDashboardData && Array.isArray(safeDashboardData.licenseUtilizationData)) ? safeDashboardData.licenseUtilizationData : [];

  const integratedProducts: IntegratedProduct[] = [
    {
  ...
  const alertStats = (safeDashboardData && Array.isArray(safeDashboardData.alertStats) ? safeDashboardData.alertStats : []).map(stat => ({
    ...stat,
    icon: (stat && typeof stat === 'object' && stat.title && typeof stat.title === 'string') ? (
      stat.title.includes('Critical') ? AlertCircle :
  ...
  const renderTooltip = (props: any) => {
    if (props.active && props.payload && Array.isArray(props.payload) && props.payload.length) {
                    ...
                    <PieChart>
                      ...
                      >
                        {(storageData || []).map((entry, index) => (
                          <Cell key={`cell-${index}`} fill={entry?.color || '#gray'} />
                        ))}
                      </Pie>
            ...
            <CardContent>
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {(integratedProducts || []).map((product) => {
                  const Icon = product?.icon || HardDrive;
                        ...
                        <div className="flex items-start justify-between mb-4">
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              ...
                              <Badge className={product.statusColor || 'bg-gray-100 text-gray-800'}>
                  ...
                  <Card key={index} className="border-0 shadow-md">
                    <CardContent className="p-4">
                      <div className="flex items-center justify-between">
                        ...
                        <div className={`flex items-center justify-center w-10 h-10 bg-gradient-to-br ${alert.color || 'from-gray-500 to-gray-600'} rounded-xl`}>
                          <Icon className="w-5 h-5 text-white" />
                        </div>
                      </div>
                    </CardContent>
