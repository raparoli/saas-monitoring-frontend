// Shared API service
import { User, AlertEmailLog, ProductDetail, IntegrationProduct } from '../types';
...
class ApiService {
  ...
  async getDashboardStats(): Promise<{ data: any; success: boolean; message?: string }> {
  ...
  async getAvailableProducts(): Promise<{ data: IntegrationProduct[]; success: boolean; message?: string }> {
    await this.mockDelay(600);
